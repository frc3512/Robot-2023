...
public class SwerveParser {
  ...
  private static void checkDirectory(File directory) {
    ...
    assert new File(directory, "modules/back").exists()
        && new File(directory, "modules/back").isDirectory();
    assert new File(directory, "modules/front").exists()
        && new File(directory, "modules/front").isDirectory();
  ...
  private static MotorController createMotor(JsonNode motorConfig) {
    ...
    switch (motorConfig.get("Type").asText()) {
      ...
      case "Falcon":
        return motorConfig.has("CANBus")
            ? new WPI_TalonFX(motorID, motorConfig.get("CANBus").asText())
  ...
  private static Object createEncoder(JsonNode encoderConfig) {
    ...
    switch (encoderConfig.get("Type").asText()) {
      case "CANCoder":
        return encoderConfig.has("CANBus")
            ? new CANCoder(encoderID, encoderConfig.get("CANBus").asText())
  ...
  private static WPI_Pigeon2 createGyro(JsonNode gyroConfig) {
    ...
    if (gyroConfig.get("Type").asText().equals("Pigeon2")) {
      return gyroConfig.has("CANBus")
          ? new WPI_Pigeon2(gyroID, gyroConfig.get("CANBus").asText())
  ...
  private static void checkModule(JsonNode moduleJson) {
    ...
    assert moduleJson.get("Motor").get("Drive").has("Type")
        && moduleJson.get("Motor").get("Drive").has("ID");
    assert moduleJson.get("Motor").has("Steer");
    assert moduleJson.get("Motor").get("Steer").has("Type")
        && moduleJson.get("Motor").get("Steer").has("ID")
  ...
  private static void checkMain(JsonNode mainJson) {
    ...
    assert mainJson.get("Speed").has("MetersPerSecond")
        && mainJson.get("Speed").has("RadianPerSecond")
        && mainJson.get("Speed").has("PhysicalMetersPerSecond");
    assert mainJson.has("Acceleration");
    assert mainJson.get("Acceleration").has("MetersPerSecond")
        && mainJson.get("Acceleration").has("RadianPerSecond");
    assert mainJson.has("Drive");
    assert mainJson.get("Drive").has("Inverted")
        && mainJson.get("Drive").has("GearRatio")
        && mainJson.get("Drive").has("MaxPower");
    assert mainJson.has("Steer");
    assert mainJson.get("Steer").has("Inverted")
        && mainJson.get("Steer").has("GearRatio")
        && mainJson.get("Steer").has("MaxPower");
    assert mainJson.has("Gyro");
    assert mainJson.get("Gyro").has("Inverted")
        && mainJson.get("Gyro").has("Type")
        && mainJson.get("Gyro").has("ID");
    assert mainJson.has("Initial Pose");
    assert mainJson.get("Initial Pose").has("X")
        && mainJson.get("Initial Pose").has("Y")
  ...
  private static SwerveModule<?, ?, ?> createModule(
      JsonNode mainJson, File moduleFile, SwerveModuleLocation moduleLocation) {
    ...
    if (encoder == null) {
      if (moduleJson.get("IntegratedAbsoluteEncoder").asBoolean()
          && (steerMotor instanceof CANSparkMax)) {
    ...
    if (moduleJson.get("Motor").get("Steer").has("CurrentLimit")) {
      module.turningMotor.setCurrentLimit(
          moduleJson.get("Motor").get("Steer").get("CurrentLimit").asInt());
    ...
    if (moduleJson.get("Motor").get("Drive").has("CurrentLimit")) {
      module.driveMotor.setCurrentLimit(
          moduleJson.get("Motor").get("Drive").get("CurrentLimit").asInt());
  ...
  private static void setModulePIDF(SwerveMotor motor, JsonNode jsonPIDF) {
    if (jsonPIDF.has("P")
        ...
        && jsonPIDF.has("D")
        && jsonPIDF.has("F")
        && jsonPIDF.has("IntegralZone")) {
      motor.setPIDF(
          jsonPIDF.get("P").asDouble(),
  ...
  public static SwerveDrive fromJSONDirectory(File directory) {
    ...
    return new SwerveDrive(
        ...
        new Pose2d(
            Units.inchesToMeters(swerveJson.get("Initial Pose").get("X").asDouble()),
            Units.inchesToMeters(swerveJson.get("Initial Pose").get("Y").asDouble()),
            Rotation2d.fromDegrees(swerveJson.get("Initial Pose").get("Rotation").asDouble())));
  }
}
