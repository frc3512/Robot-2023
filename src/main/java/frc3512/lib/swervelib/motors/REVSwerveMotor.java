...
public class REVSwerveMotor extends SwerveMotor {
  ...
  /**
   * Constructor for REV Swerve Motor, expecting CANSparkMax. Clears sticky faults and restores
   * factory defaults.
   *
   * @param motor SparkMAX motor controller.
   * @param absoluteEncoder The absolute encoder used for the module, if the motor is a turning
   *     motor and the encoder is compatible it will set the encoder as the remote integrated
   *     encoder and does not need periodic synchronization.
  ...
  public REVSwerveMotor(
    ...
    m_integratedAbsEncoder =
        absoluteEncoder.m_encoder instanceof AbsoluteEncoder && type == ModuleMotorType.TURNING;
    ...
    m_motorType = type;
    m_encoder =
        m_integratedAbsEncoder ? (AbsoluteEncoder) absoluteEncoder.m_encoder : motor.getEncoder();
  ...
  public void setConversionFactor(double conversionFactor) {
    if (m_motorType == ModuleMotorType.TURNING) {
      ...
      // SparkMaxSimProfile assumes the velocity is in RPM and multiplies it by 60, in our use case
      // velocity is in RPS
  ...
  public void setTarget(double target, double feedforward) {
    this.target = target;
    m_pid.setReference(
        target, m_pidControlType, m_mainPidSlot, feedforward * m_moduleRadkV, m_feedforwardUnits);
  }
   ...
   */
  public void setCANStatusFrames(
  ...
  public boolean reachable() {
    // Based off of
    // https://github.com/DigitalDislocators/SDS-MK4i-NEO-Swerve-Template/blob/main/src/main/java/frc/robot/subsystems/SwerveModuleSparkMax.java#L490
