...
public class SwerveModule<
        DriveMotorType extends MotorController,
        AngleMotorType extends MotorController,
        AbsoluteEncoderType>
  ...
  /**
   * Swerve module constructor. Both motors <b>MUST</b> be a {@link MotorController} class. It is
   * recommended to create a command to reset the encoders when triggered and
   *
   ...
   * @param driveGearRatio Drive gear ratio in form of (rotation:1 AKA rotations/1) to get the
   *     encoder ticks per rotation.
   * @param steerGearRatio Steering motor gear ratio (usually 12.8:1 for MK4 in form of rotations:1
   *     or rotations/1), only applied if using Neo's.
  ...
  public SwerveModule(
      ...
      SwerveModuleLocation swervePosition,
    ...
    // 9.  Set velocity and position conversion factors on turning motor controller. (Usually
    // P=0.01,I=0,D=0,F=0,IZ=1)
    ...
    assert encoder instanceof DutyCycleEncoder
        || encoder instanceof AnalogEncoder
        || encoder instanceof CANCoder
    ...
    //        public static final double KV = 12 / MAX_SPEED; // Volt-seconds per meter (max voltage
    // divided by max
    //        speed)
    //        public static final double KA = 12 / MAX_ACCELERATION; // Volt-seconds^2 per meter
    // (max voltage divided
    //        by max accel)
    maxDriveSpeedMPS = maxSpeedMPS;
    driveFeedforward =
        new SimpleMotorFeedforward(0, 12 / maxDriveSpeedMPS, 12 / maxDriveAcceleration);
  ...
  /**
   * Reset the REV encoders onboard the SparkMax's to 0, and set's the drive motor to position to 0
   * and synchronizes the internal steering encoders with the absolute encoder.
   */
  ...
  public void synchronizeSteeringEncoder() {
    if (!remoteIntegratedEncoder()) {
      if (absoluteEncoder.getMagnetFieldStrength() != MagnetFieldStrength.Good_GreenLED) {
        System.err.println(
            "CANCoder magnetic field strength is unacceptable, will not synchronize encoders.");
        return;
      }
   ...
   *
   * <p><b>P</b> = .5 and increase it by .1 until oscillations occur, then decrease by .05 then .005
   * until oscillations stop and angle is perfect or near perfect.
   *
   * <p><b>I</b> = 0 tune this if your PID never quite reaches the target, after tuning <b>D</b>.
   * Increase this by <b>P</b>*.01 each time and adjust accordingly.
   *
   * <p><b>D</b> = 0 tune this if the PID accelerates too fast, it will smooth the motion. Increase
   * this by <b>P</b>*10 and adjust accordingly.
   *
   * <p><b>F</b> = 0 tune this if the PID is being used for velocity, the <b>F</b> is multiplied by
   * the target and added to the voltage output. Increase this by 0.01 until the PIDF reaches the
   * desired state in a fast enough manner. Documentation for this is best described by CTRE <a
  ...
  public boolean activeCAN() {
    boolean drive = driveMotor.reachable(),
        turn = turningMotor.reachable(),
        encoder = absoluteEncoder.reachable();

    return drive && turn && encoder;
  }

  /**
   * Checks if the absolute encoder is able to be read directly by the motor controller instead of
   * using the synchronizeEncoder() function periodically.
   *
   * @return If the absolute sensor in the steering motor is directly accessed by the motor
   *     controller.
   */
  ...
  public void setVelocity(double velocity) {
    driveMotor.setTarget(
        velocity, driveFeedforward.calculate(velocity) / RobotController.getBatteryVoltage());
  }
  ...
  public SwerveModuleState2 getState() {
    ...
    angle =
        Rotation2d.fromDegrees(
            Math.round(
                Robot.isReal()
                    ? absoluteEncoder.getAbsolutePosition() - angleOffset
                    : targetAngle));
    angularVelocityRPS =
        Robot.isReal() ? Math.toRadians(absoluteEncoder.getVelocity()) : targetAngularVelocityRPS;
  ...
  public void setState(SwerveModuleState2 state) {
    ...
    // inspired by
    // https://github.com/first95/FRC2022/blob/1f57d6837e04d8c8a89f4d83d71b5d2172f41a0e/SwervyBot/src/main/java/frc/robot/SwerveModule.java#L22
    Rotation2d currentAngle = getState().angle;
    //    SwerveModuleState optimizedState = SwerveModuleState.optimize(new
    // SwerveModuleState(state.speedMetersPerSecond,
    //
    ...
    angle =
        (Math.abs(optimizedState.speedMetersPerSecond) <= (maxDriveSpeedMPS * 0.01))
            ? targetAngle
            : angle;
   ...
   *
   * @return Swerve Module position, with the angle as what the angle is supposed to be, not what it
   *     actually is.
   */
   ...
   *
   * @param level Verbosity level, affects the CAN utilization, on HIGH it will enable the update
   *     button.
   */
      ...
      case SETUP:
        ...
        if (goodLaptop) {
          addEntry(
                  "Angle (Degrees)",
                  new Double[] {
                    targetAngle, turningMotor.get(), absoluteEncoder.getAbsolutePosition()
                  })
              .withPosition(0, 1)
              .withSize(4, 4)
          ...
          addEntry("Absolute Angle", absoluteEncoder.getAbsolutePosition())
              .withPosition(0, 0)
              .withSize(2, 1);
          addEntry("Integrated Angle", turningMotor.get()).withPosition(2, 0).withSize(2, 1);
          addEntry("Target Angle", targetAngle).withPosition(8, 4).withSize(2, 2);
          ...
          addEntry("Velocity (Meters Per Second)", new Double[] {targetVelocity, driveMotor.get()})
              ...
              .withProperties(Map.of("visible time", 5))
              .withWidget(BuiltInWidgets.kGraph);
          addEntry("Motor Velocity", driveMotor.get()).withPosition(4, 0).withSize(4, 1);
          addEntry("Target Velocity", targetVelocity).withPosition(8, 2).withSize(2, 2);

          // Inverted Motors.
          addEntry("Steer Motor Inverted", invertedTurn).withPosition(0, 5).withSize(4, 1);
          addEntry("Drive Motor inverted", invertedDrive).withPosition(4, 5).withSize(4, 1);

          addEntry(
                  "Absolute Encoder Magnetic Field", absoluteEncoder.getMagnetFieldStrength().value)
              .withPosition(10, 2)
              .withSize(2, 1);
          addEntry("Drive Motor Current", driveMotor.getAmps()).withPosition(10, 5).withSize(2, 1);
          ...
          // Inverted Motors.
          addEntry("Steer Motor Inverted", invertedTurn).withPosition(0, 4).withSize(4, 1);
          addEntry("Drive Motor inverted", invertedDrive).withPosition(4, 4).withSize(4, 1);
          ...
          addEntry("Drive Motor Current", driveMotor.getAmps()).withPosition(10, 2).withSize(2, 1);
        ...
        addEntry("CAN Connection", activeCAN()).withPosition(8, 0).withSize(4, 2);
  ...
  public void initSendable(SendableBuilder builder) {
    builder.setSmartDashboardType(
        SwerveModuleLocationToString(swerveLocation) + " SwerveDriveModule");
  ...
  private Translation2d getSwerveModulePosition(SwerveModuleLocation swerveLocation) {
    // Modeling off of
    // https://github.com/Stampede3630/2022-Code/blob/master/src/main/java/frc/robot/SwerveDrive.java
    switch (swerveLocation) {
      case FrontLeft:
  ...
  public enum Verbosity {
    /**
     * The bare minimum and not utilize the CAN bus when reporting data. Only posts data from
     * attributes.
     */
    LOW,
