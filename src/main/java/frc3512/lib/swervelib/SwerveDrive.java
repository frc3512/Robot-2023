...
import frc3512.lib.swervelib.SwerveModule.SwerveModuleLocation;
...
/**
 * SwerveDrive base which is meant to be platform agnostic. This implementation expect second order
 * kinematics because second order kinematics prevents the drift that builds up when using first
 * order kinematics. As per this <a
...
public class SwerveDrive extends RobotDriveBase implements Sendable, AutoCloseable {
  ...
  /**
   * Constructor for Swerve Drive assuming modules have been created and configured with PIDF and
   * conversions.
   *
   ...
   * @param driverMaxAngularVelocityRadiansPerSecond Maximum angular velocity for turning when using
   *     the drive function.
   * @param driverMaxDriveAccelerationMetersPerSecond Maximum acceleration in meters per second for
   *     the drive motors when in teleop for the slew rate limiter.
   * @param driverMaxAngularAccelerationRadiansPerSecond Maximum angular acceleration in meters per
   *     second for the steering motors when in teleop for the slew rate limiters.
   * @param physicalMaxSpeedMPS Maximum speed a module can go physically, used to desaturate wheel
   *     speeds.
  ...
  public SwerveDrive(
    ...
    m_swerveKinematics =
        new SwerveDriveKinematics2(
            frontLeft.swerveModuleLocation,
    ...
    // Inspired by
    // https://github.com/Team364/BaseFalconSwerve/blob/main/src/main/java/frc/robot/subsystems/Swerve.java
    ...
    for (int i = 0; i < configs.length; i++) {
      ...
      modules[loc] =
          configs[i].createModule(
              ...
              steeringMotorInverted,
              drivingMotorInverted);
    }
    return modules;
  }
    ...
    if (!Robot.isReal()) {
      ...
      m_field
          ...
          .setPoses(
              pos.plus(
                  new Transform2d(
                      m_frontLeft.swerveModuleLocation, m_frontLeft.getPosition().angle)),
              pos.plus(
                  new Transform2d(
                      m_frontRight.swerveModuleLocation, m_frontRight.getPosition().angle)),
              pos.plus(
                  new Transform2d(m_backLeft.swerveModuleLocation, m_backLeft.getPosition().angle)),
              pos.plus(
                  new Transform2d(
                      m_backRight.swerveModuleLocation, m_backRight.getPosition().angle)));
    }
  ...
  /** Adds vision measurements into the pose estimator. */
  public void addVisionMeasurement(Pose2d pose, double timestamp) {
    m_swervePoseEstimator.addVisionMeasurement(pose, timestamp);
  }
  ...
  public void set(double forward, double strafe, double radianPerSecond, boolean fieldRelative) {
    ChassisSpeeds node =
        fieldRelative
            ? ChassisSpeeds.fromFieldRelativeSpeeds(forward, strafe, radianPerSecond, getRotation())
  ...
  public void set(ChassisSpeeds node) {
    ...
    // Taken from
    // https://www.chiefdelphi.com/t/whitepaper-swerve-drive-skew-and-second-order-kinematics/416964/5?u=nstrike
    double timerDt = (m_driveTimer.get() - m_timerPrev);
    Pose2d robot_pose_vel =
        new Pose2d(
            node.vxMetersPerSecond * timerDt,
    ...
    SwerveModuleState2[] moduleStates =
        m_swerveKinematics.toSwerveModuleStates(updated_chassis_speeds);
    //        new Translation2d((m_frontLeft.swerveModuleLocation.getX() +
    // m_frontRight.swerveModuleLocation.getX()) / 2,
    //                          (m_frontLeft.swerveModuleLocation.getY() +
    // m_backLeft.swerveModuleLocation.getY()) / 2));
  ...
  /**
   * Set the swerve module states given an array of states. Normalize the wheel speeds to abide by
   * maximum supplied
   *
   * @param states Module states in a specified order. [front left, front right, back left, back
   *     right]
  ...
  public Rotation2d getRotation() {
    if (Robot.isReal()) {
      return m_gyroInverted
          ? Rotation2d.fromDegrees(360 - m_pigeonIMU.getYaw())
          : Rotation2d.fromDegrees(m_pigeonIMU.getYaw());
    } else {
  ...
  public static class SwerveModuleConfig<
    ...
    /**
     * Swerve Module configuration class to define the motor CAN IDs and absolute encoder offset of
     * a swerve module.
     *
     ...
     * @param driveGearRatio Drive gear ratio in form of (rotation:1 AKA rotations/1) to get the
     *     encoder ticks per rotation.
     * @param steerGearRatio Steering motor gear ratio (usually 12.8:1 for MK4 in form of
     *     rotations:1 or rotations/1), only applied if using Neo's.
     * @param wheelDiameterMeters The wheel diameter of the swerve drive module in meters.
     * @param wheelBaseMeters The Distance between front and back wheels of the robot in meters.
     * @param driveTrainWidthMeters The Distance between centers of right and left wheels in meters.
     * @param steeringMotorFreeSpeedRPM The RPM free speed of the steering motor.
     ...
     */
    public SwerveModule<DriveMotorType, SteeringMotorType, AbsoluteEncoder> createModule(
        double driveGearRatio,
        double steerGearRatio,
        double wheelDiameterMeters,
        double wheelBaseMeters,
